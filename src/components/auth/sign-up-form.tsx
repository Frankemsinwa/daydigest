...
export function SignUpForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (error) {
      ...
      // This can happen if "Confirm email" is turned on in Supabase settings and user already exists but is not confirmed
      toast({
      ...
      // User is signed up and logged in (e.g. if auto-confirm is on or email is already verified)
      toast({
      ...
      // User is created, but confirmation email is sent (default Supabase behavior)
      toast({
        title: 'Sign Up Successful',
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="•••••••• (min. 6 characters)" {...field} type="password" disabled={isLoading} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
